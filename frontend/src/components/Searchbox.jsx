    ...
    return (
        ...
        <div className="bg-gray-100 p-4">
            ...
            <div className="flex justify-center my-4">
                <button
                    className={`bg-transparent border-2 text-xl font-bold border-cyan-700 text-cyan-700 px-4 py-2 rounded-full hover:bg-cyan-100 mr-2 ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                    onClick={handlePreviousPage}
                    disabled={currentPage === 1}
                >
                    <svg className="h-5 w-5 text-cyan-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </button>
                <span className="text-xl font-bold mx-4">Page {currentPage}</span>
                <button
                    className={`bg-transparent border-2 text-xl font-bold border-cyan-700 text-cyan-700 px-4 py-2 rounded-full hover:bg-cyan-100 mr-2 ${indexOfLastItem >= filteredNgos.length ? 'opacity-50 cursor-not-allowed' : ''}`}
